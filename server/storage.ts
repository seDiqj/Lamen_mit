...
export class DatabaseStorage implements IStorage {
  ...
  async getCollectionInstallments(filters: { filter?: string; branch?: string; officer?: string; search?: string; startDate?: string; endDate?: string; page?: number; limit?: number }): Promise<{ installments: any[]; total: number; summary: any }> {
    ...
    if (filter === "upcoming") {
      conditions.push(sql`${installments.isPaid} = false AND ${installments.dueDate}::date > ${today}::date AND ${installments.dueDate}::date <= ${fiveDaysLater}::date`);
    } else if (filter === "due_soon") {
      conditions.push(sql`${installments.isPaid} = false AND ${installments.dueDate}::date <= ${fiveDaysLater}::date`);
    ...
    const summaryResults = await db
      .select({
        totalDue: sql<string>`COALESCE(SUM(CASE WHEN ${installments.isPaid} = false AND ${installments.dueDate}::date <= ${fiveDaysLater}::date THEN ${installments.totalAmount}::numeric ELSE 0 END), 0)`,
        totalCollected: sql<string>`COALESCE(SUM(CASE WHEN ${installments.isPaid} = false AND ${installments.dueDate}::date <= ${fiveDaysLater}::date THEN COALESCE(${installments.paidAmount}::numeric, 0) ELSE 0 END), 0)`,
        totalRemaining: sql<string>`COALESCE(SUM(CASE WHEN ${installments.isPaid} = false AND ${installments.dueDate}::date <= ${fiveDaysLater}::date THEN (${installments.totalAmount}::numeric - COALESCE(${installments.paidAmount}::numeric, 0)) ELSE 0 END), 0)`,
        ...
        upcomingCount: sql<number>`COUNT(CASE WHEN ${installments.isPaid} = false AND ${installments.dueDate}::date > ${today}::date AND ${installments.dueDate}::date <= ${fiveDaysLater}::date THEN 1 END)`,
  ...
  async getDashboardStats(filters?: { branchId?: string; startDate?: string; endDate?: string }): Promise<any> {
    ...
    const productBreakdownResult = await db.execute(sql`
      WITH per_loan AS (
        SELECT
          l.id,
          COALESCE(l.product_name, 'Unknown') AS product_name,
          COALESCE(l.principle_amount::numeric, l.request_amount::numeric, 0) AS principal,
          COALESCE(l.profit::numeric, 0) AS margin,
          LEAST(
            COALESCE((
              SELECT SUM(GREATEST(COALESCE(i.paid_amount::numeric, 0), 0))
              FROM installments i
      ...
      allocated AS (
        SELECT
          *,
          LEAST(collected, margin) AS margin_collected,
          LEAST(
            GREATEST(collected - LEAST(collected, margin), 0),
            principal
      ...
      SELECT
        ...
        COALESCE(SUM(principal + margin), 0) AS total_portfolio,
        COALESCE(SUM(principal), 0) AS portfolio_principal,
        COALESCE(SUM(margin), 0) AS portfolio_margin,
        COALESCE(SUM(principal_collected + margin_collected), 0) AS total_collected,
        COALESCE(SUM(principal_collected), 0) AS principal_collected,
        COALESCE(SUM(margin_collected), 0) AS margin_collected,
        COALESCE(SUM(GREATEST(principal - principal_collected, 0)), 0) AS outstanding_principal,
        COALESCE(SUM(GREATEST(margin - margin_collected, 0)), 0) AS outstanding_margin,
        COALESCE(SUM(
          GREATEST(principal - principal_collected, 0)
            + GREATEST(margin - margin_collected, 0)
        ), 0) AS outstanding_balance
      FROM allocated
      GROUP BY product_name
    `);

    const productBreakdown = (productBreakdownResult.rows as any[])
      .map(r => ({
        productName: r.product_name,
        loanCount: Number(r.loan_count),
        totalDisbursed: Number(r.total_disbursed),
        totalPortfolio: Number(r.total_portfolio),
        ...
        principalCollected: Number(r.principal_collected),
        marginCollected: Number(r.margin_collected),
        outstandingBalance: Number(r.outstanding_balance),
        outstandingPrincipal: Number(r.outstanding_principal),
        outstandingMargin: Number(r.outstanding_margin),
      }))
